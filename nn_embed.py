...
df_sub = pd.DataFrame({'target' : y_pred_final})
df_sub.to_csv('oof/nn_embed_train.csv', index=False)

pd.DataFrame(full_val_preds).to_csv('oof/nn_embed_train.csv',index=False)